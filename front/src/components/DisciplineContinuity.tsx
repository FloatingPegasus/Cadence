...
function DisciplineContinuity({
  ...
  useEffect(() => {
    ...
    fetchDisciplineMonthlyContinuity(disciplineId, month)
      ...
      .catch((caught) => {
        if (active) {
          setError(caught instanceof Error ? caught.message : "Could not load discipline continuity");
        }
      });
    return () => {
      active = false;
  ...
  return (
    <section className="mt-6 border border-neutral-800 bg-neutral-950/40 px-4 py-4" aria-labelledby="discipline-continuity-title">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-wider text-neutral-500">Discipline continuity</p>
          <h2 id="discipline-continuity-title" className="mt-1 text-base font-medium text-neutral-100">
            {data?.discipline.name ?? "Loading discipline"}
          </h2>
          <p className="mt-1 text-sm text-neutral-500">Completion days and the trace attached to them.</p>
        </div>
      ...
      {!data && !error && <p className="mt-4 text-sm text-neutral-500">Loading continuity…</p>}
      {data && (
        <>
          <dl className="mt-5 grid grid-cols-3 gap-3 border-y border-neutral-800 py-3 text-sm">
            ...
            <div><dt className="text-neutral-500">Linked trace days</dt><dd className="mt-1 text-neutral-100">{data.totals.linked_trace_days}</dd></div>
            <div><dt className="text-neutral-500">Contexts</dt><dd className="mt-1 text-neutral-100">{data.totals.contexts}</dd></div>
          </dl>
            ...
            {data.days.map((day) => (
              <button key={day.date} type="button" onClick={() => onSelectDate(day.date)} aria-current={selectedDate === day.date ? "date" : undefined} className="block w-full border-b border-neutral-800/70 py-2 text-left transition-colors duration-150 hover:bg-neutral-900 focus-visible:outline focus-visible:outline-2 focus-visible:outline-violet-400">
                <span className="flex items-baseline justify-between gap-3">
                  ...
                  <span className="text-xs text-neutral-600">{day.conversation_entries} conversation {day.conversation_entries === 1 ? "entry" : "entries"}</span>
                </span>
          ...
          {data.contexts.length > 0 && (
            <div className="mt-5 border-t border-neutral-800 pt-4">
              <p className="text-xs uppercase tracking-wider text-neutral-500">Linked contexts</p>
