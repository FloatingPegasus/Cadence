    ...
    >
      ...
      {!user?.ai_processing_consent && (
        <p className="mt-3 text-xs text-neutral-600">
          External generation is off. Write locally or enable AI in Settings.
        </p>
      )}
      ...
      {reflection && (
        <p className="mt-2 text-[11px] text-neutral-600">
          {reflection.is_user_edited
            ? "Manually edited"
            : `Generated by ${reflection.model ?? reflection.provider}`}
          {" · "}
          source {reflection.source_fingerprint.slice(0, 10)}
        </p>
      )}
      {error && (
